 ...
 * Name: Caleb Matherne
 * Date: 10/4/2022
 * Description: Program 3: Stacks, Queues and Generics
 */
...
public class Main {
  ...
  private static Stack<Character> stack; //private static Stack stack;
  private static Queue<Character> queue; //private static Queue queue;
  ...
  public static void main(String[] args) {
    stack = new Stack<Character>(); //stack = new Stack();
    queue = new Queue<Character>(); //queue = new Queue();
    ...
    switch(choice) {
      ...
      case 'r':
        if (!queue.isEmpty()) {
          playerRow = PLAYER_START_ROW;
          playerCol = PLAYER_START_COL;
          Queue<Character> newQueue = new Queue<Character>(); //Queue newQueue = new Queue();
          while(!queue.isEmpty()) {
            clearScreen();
            draw();
            System.out.flush();
